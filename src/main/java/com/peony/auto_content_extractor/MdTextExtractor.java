 ...
 * 
 * @see https://github.com/abguorui0928/cx-extractor
 * 
 * @author Fandy Wang(lfwang@ir.hit.edu.cn)
 * @date 2010.08.12
 */
public class MdTextExtractor {
	...
	public void extractURL(String url) {
		...
		if (isLegalURL(url)) {
			if (isContentURL(url)) {
				try {
					String htmlText = HttpQuery.getInstance().get(url).asString();
	...
	public void extractURL(String url, String encoding) {
		...
		if (isLegalURL(url)) {
			if (isContentURL(url)) {
				try {
					String htmlText = HttpQuery.getInstance().get(url).asString(encoding);
					extractHTML(htmlText);
				} catch (BaseHttpException e) {
					_text = "*请求网页源码失败！:-)";
				}
